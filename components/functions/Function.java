...
public abstract class Function extends JavmosComponent {
    ...
    public void draw(Graphics2D graphics2D) {
        ...
        double bounds = positionalOffset / zoom; 
        double startingPoint = Math.max(minDomain, -bounds);
        double endPoint = Math.min(maxDomain, bounds);
        
        for (double xValue = startingPoint; xValue < endPoint; xValue += precision) {
            ...
            if (!Double.isNaN(yValue)) {
                if ((yValue >= minRange) & (yValue <= maxRange)) {
